...
<form method="POST" action="{{ route('checkouts.devices.patch', $device->id) }}">
  @csrf
  @method('PATCH')
  ...
  <div class="max-w-2xl border-b border-gray-900/10 mx-auto bg-white p-8 rounded-lg shadow-md">
    <div class="pb-12">
      <div class="mt-10 flex flex-col gap-y-2">
        ...
        {{-- Replaced plain-text model_number input with relational model select --}}
        <div class="">
          <x-checkouts.model-select :selected="$device->computerModel" required />
        ...
        <div class="">
          <x-checkouts.pool-select :selected="$device->pool_id" :pools="$pools"/>
          @error('pool_id')
            <p class="text-xs text-red-500 font-semibold mt-1">{{ $message }}</p>
          @enderror
        </div>
...
<form id="delete-device-form" method="POST" action="{{ route('checkouts.devices.delete', $device->id) }}" class="hidden">
  @csrf
  @method('DELETE')
  <input type="hidden" name="return_url" value="{{ $returnUrl }}">
</form>
