<x-layout>
  ...
  <form method="POST" action="{{ isset($status) ? route('taxonomy.status.update', $status) : route('taxonomy.status.store') }}">
    @csrf
    @if(isset($status))
      @method('PATCH')
    @endif
    ...
    <input type="hidden" name="tailwind_class" value="{{ $status->tailwind_class ?? 'bg-slate-500' }}">
        ...
        <div class="mt-10 flex flex-col gap-y-2">
            ...
            <div class="mt-2">
              <div class="flex items-center gap-4">
                ...
                <div id="status_preview"
                  class="inline-flex px-2 py-1 text-xs font-semibold rounded-full text-neutral-50 {{ old('tailwind_class', $status->tailwind_class ?? 'bg-gray-500') }}">
                  {{ old('status_name', $status->status_name ?? 'Status') }}
                </div>
              </div>
              @error('status_name')
          ...
          <div>
            ...
            <input type="number"
              id="weight"
              name="weight"
              value="{{ old('weight', $status->weight ?? 100) }}"
              class="border border-gray-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 w-48"
            />
          </div>
          ...
          <div class="flex flex-wrap gap-2" id="swatches-{{ $status->id ?? 'new' }}">
            @php
              $colors = config('taxonomy.colors');
              $selectedColor = old('tailwind_class', $status->tailwind_class ?? 'bg-slate-500');
            @endphp
            @foreach($colors as $color)
              <button type="button"
                onclick="selectColor('{{ $status->id ?? 'new' }}', '{{ $color }}')"
                class="swatch w-7 h-7 rounded-full {{ $color }} ring-offset-2 transition-all duration-150 {{ $selectedColor === $color ? 'ring-2 ring-gray-800' : '' }}"
                data-color="{{ $color }}">
              </button>
            @endforeach
          </div>
      ...
      <div class="mt-6 sm:flex max-w-2xl items-center justify-between">
        <div class="flex items-center gap-x-6 justify-self-end">
          <button type="submit" class="ml-3 rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">
            {{ isset($status) ? 'Update Status' : 'Add Status' }}
          </button>
